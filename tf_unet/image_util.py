...
class AdvancedDataProvider(ImageDataProvider):
    '''
    1. 载入图像时进行 augmentation
    2. 处理多标签
    '''

    def __init__(self, search_path, a_min=None, a_max=None, data_suffix=".png", mask_suffix='_mask.png', shuffle_data=True, n_class=4):
        super(AdvancedDataProvider, self).__init__(search_path, a_min,
                                                   a_max, data_suffix, mask_suffix, shuffle_data, n_class)

    def _post_process(self, data, labels):
        '''
        TODO: 进行 augmentation
        '''
        return data, labels

    def _process_labels(self, mask):
        '''
        对输入的 mask (ny, nx, 1) 转换为 (ny, nx, n_class) 的 one-shot 向量
        '''
        assert len(mask.shape) == 3, 'label 输入必须为三通道'
        nx = mask.shape[1]
        ny = mask.shape[0]

        # 剥离
        mask_r = mask[..., 0].astype('bool')
        mask_g = mask[..., 1].astype('bool')
        mask_b = mask[..., 2].astype('bool')
        mask_nr = ~mask_r
        mask_ng = ~mask_g
        mask_nb = ~mask_b

        # 转换为标签
        assert self.n_class <= 8, '只能处理不超过 2^3 个标签。'
        labels = np.zeros((ny, nx, self.n_class), dtype=np.float32)
        for i in range(self.n_class):
            labels[..., i] = (mask_r if (i & 1) else mask_nr) \
                & (mask_g if (i & 2) else mask_ng) \
                & (mask_g if (i & 4) else mask_nb)
    ...
    def _next_data(self):
        ...
        img = self._load_file(image_name, np.float32)
        label = self._load_file(label_name, np.float32)  # 读取正常的 float
